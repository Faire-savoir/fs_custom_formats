...
function format_tripadvisor( $widget_id, $options = [] ){
	...
	if ( !empty($widget_id) && is_numeric($widget_id) ) {
		//
		$uniq = rand ( 100 , 999 );
...
function format_list_reseaux_sociaux( $field = [], $field_link = '', $options = [] ){
	...
	if( is_array($field) && !empty($field) && is_string($field_link) && !empty($field_link) ) {
		...
		// Initialisation
		$field_class = ( !empty($options['field_class_label']) ) ? $options['field_class_label'] : '' ;
		$target = ( !empty($options['target']) ) ? $options['target'] : '_blank' ;
		...
		foreach( $field as $item) {
			if ( !empty($item[$field_link]) ) {
				...
				$label = '';
				$specific_class = '';
				if ( !empty($item[$field_class]) && function_exists('get_label_thesaurus_by_id') ) {
